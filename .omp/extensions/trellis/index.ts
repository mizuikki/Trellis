...
function resolveProjectFile(projectRoot: string, file: string): string | null {
   ...
   try {
      ...
      if (!isInsideRoot(rootReal, candidate)) return null;
      let target = candidate;
      try { target = realpathSync(candidate); } catch { }
      return isInsideRoot(rootReal, target) ? target : null;
   } catch {
      return null;
   }
}
...
function readBoundedArtifact(path: string, displayPath: string): string {
   ...
   if (!bytes) return "";
   if (bytes.length <= MAX_TASK_ARTIFACT_BYTES) return new StringDecoder("utf8").write(bytes);
   const suffix = Buffer.from(
...
function normalizeReason(value: unknown): string {
   ...
   return reason.length <= MAX_REASON_CHARS ? reason : reason.slice(0, MAX_REASON_CHARS - 3) + "...";
}

function renderManifestIndex(projectRoot: string, taskDir: string, jsonlName: string): string {
   ...
   for (const line of source.split(/\r?\n/)) {
      ...
      try {
         ...
         const displayPath = relative(resolve(projectRoot), resolve(projectRoot, rawPath.replace(/\\/g, "/"))).replace(/\\/g, "/");
         const key = `${entryType}:${displayPath}`;
...
function buildTaskContext(projectRoot: string, taskDir: string, agentType?: AgentType): string {
   const displayTaskDir = relative(projectRoot, taskDir).replace(/\\/g, "/") || ".";
   const parts: string[] = [`Task directory: ${displayTaskDir}`];
   ...
   for (const jsonlName of jsonlNames) {
      const index = renderManifestIndex(projectRoot, taskDir, jsonlName);
   ...
   for (const [name, label] of [
      ...
      const content = readBoundedArtifact(join(taskDir, name), displayPath);
      if (content.trim()) parts.push(`## ${displayPath} (${label})\n\n${content.trim()}`);
   }
