...
- `status=planning` + `prd.md` only → decide whether the task is lightweight or complex. Lightweight can move to **1.4** review; complex returns to **1.1** and runs `{{PYTHON_CMD}} ./.trellis/scripts/task.py scaffold <task> all` for missing artifacts.
