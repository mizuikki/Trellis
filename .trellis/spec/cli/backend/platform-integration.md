...
## Scenario: Kimi Code Platform Contract

### 1. Scope / Trigger

Apply this contract when adding or changing Kimi registry metadata, generated
skills, Python adapter behavior, or workflow routing.

### 2. Signatures

- Configure a project: `trellis init --kimi`
- Run non-interactively: `kimi -p <prompt>`
- Resume a session: `kimi --session <session-id>`
- Invoke a generated entry point: `/skill:trellis-<name>`

### 3. Contracts

- Shared workflow and bundled skills are rendered neutrally under
  `.agents/skills/`.
- Session commands and Trellis role prompts are written under
  `.kimi-code/skills/<name>/SKILL.md`.
- `configureKimi()` and `collectKimiTemplates()` produce the same paths and
  bytes.
- Kimi is agent-capable and pull-based, with no project hook, settings, or
  custom-agent file generated by Trellis.

### 4. Validation & Error Matrix

| Condition | Expected behavior |
| --- | --- |
| `.kimi-code/` is the configured platform directory | Detection returns `kimi`. |
| A run prompt is built | Arguments are exactly `kimi`, `-p`, and the prompt. |
| A session ID is resumed | Arguments are exactly `kimi`, `--session`, and the ID. |
| Kimi and another shared-skill platform are configured | `.agents/skills/` content remains byte-identical regardless of write order. |
| Kimi templates are collected for update tracking | Every collected file exists with identical bytes after configuration. |

### 5. Good/Base/Bad Cases

- Good: Kimi and Pi both write the same neutral `.agents/skills/` bytes while
  Kimi-specific commands remain private to `.kimi-code/skills/`.
- Base: Kimi alone produces shared workflow skills, private command/role
  skills, and no hooks/settings surface.
- Bad: using `kimi -p <prompt> --yolo`; current Kimi rejects prompt mode
  combined with `--yolo`.

### 6. Tests Required

- Registry assertions for config directory, CLI flag, capability flags, and
  `/skill:trellis-` references.
- Init/configurator tests for generated paths, byte parity, and absence of
  hooks/settings files.
- Python regression tests asserting the exact run/resume argument arrays,
  platform detection, command paths, and task-context seeding.
- Workflow and shared-skill parity tests across canonical and generated
  copies.
...
| Command       | `agentCapable && hasHooks` (12)                                                                                                                                                                                                                | `agentCapable && !hasHooks` (5)                                                                       | `!agentCapable` (3)                                  |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
