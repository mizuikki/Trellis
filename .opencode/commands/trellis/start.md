...
- **Active task status `planning` + `prd.md` exists** → stay in Phase 1. Lightweight tasks can be PRD-only. For complex tasks, scaffold missing artifacts with `python3 ./.trellis/scripts/task.py scaffold <task> all`; any non-regular (including a symlink), unreadable, empty, or first-five-line `<!-- trellis:scaffold-unfilled -->` artifact is pending, never complete. Fill and review Core and triggered semantics before removing each sentinel, then load the relevant Phase 1 step detail before `task.py start`.
